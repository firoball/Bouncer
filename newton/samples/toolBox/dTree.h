...
//	static dInt32 m_size;
	...
	bool SanityCheck (dTreeNode *ptr, dInt32 height) const;
...
template<class OBJECT, class KEY>
dTree<OBJECT, KEY>::operator dInt32() const
{
	return m_head != NULL;
}

template<class OBJECT, class KEY>
dInt32 dTree<OBJECT, KEY>::GetCount() const
{
	return m_count;
}
...
template<class OBJECT, class KEY>
bool dTree<OBJECT, KEY>::SanityCheck (typename dTree<OBJECT, KEY>::dTreeNode *ptr, dInt32 height) const
{
	dInt32 bh;
	dTreeNode *x;
...
template<class OBJECT, class KEY>
dInt32 dTree<OBJECT, KEY>::CompareKeys (const KEY &key0, const KEY &key1) const
{
	if (key1 < key0) {
		return - 1;
	}
...
//template<class OBJECT, class KEY> dInt32 dTree<OBJECT,KEY>::m_size = 0;
